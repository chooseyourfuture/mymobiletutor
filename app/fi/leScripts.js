	...
	document.getElementById('customAddressInputButton').addEventListener('click', function() {
		...
		if(customAddress == "" || customAddressName == ""){
			...
			<div class="alert alert-warning alert-dismissible fade show" role="alert">
			  <strong>Lisää nimi/osoite</strong>
			</div>`
		}else{
			...
			if(verifyExistsAddress(customAddressName,customAddress)){
				document.getElementById('addWarning').innerHTML = `
				<div class="alert alert-warning alert-dismissible fade show" role="alert">
				  <strong>Nimi tai osoite on jo tallennettu</strong>
				</div>`
			}else{
				document.getElementById('addWarning').innerHTML = `
				<div class="alert alert-success alert-dismissible fade show" role="alert">
				  <strong>Osoite tallennettu</strong>
				</div>`
...
//Load redirect source search if any
  ...
  function callback(results, status) {
	...
	if(status=="ZERO_RESULTS"){
		alert("Ei hakutuloksia. Tarkista oikeinkirjoitus.");
	}
  }
  ...
  function createMarker(place) {
	...
	google.maps.event.addListener(marker, 'click', function() {
		var markerContent = '<p>'+place.name+'</p><p id="addressID'+routeNums+'">'+ place.formatted_address +'</p><p><button onclick="loadContent('+routeNums+')">Reitti</button><button onclick="addToAddressBook('+routeNums+')">Tallenna paikka</button>';
		...
		if (status == 'OK') {
			...
			google.maps.event.addListener(marker, 'click', function() {
				if(manualName != undefined){
					var markerContent = '<p>'+manualName+'</p><p id="addressID'+routeNums+'">'+ results[0].formatted_address +'</p><p><button onclick="loadContent('+routeNums+')">Reitti</button><button onclick="addToAddressBook('+routeNums+')">Tallenna paikka</button>';
				}else{
					var markerContent = '<p id="addressID'+routeNums+'">'+ results[0].formatted_address +'</p><p><button onclick="loadContent('+routeNums+')">Reitti</button><button onclick="addToAddressBook('+routeNums+')">Tallenna paikka</button>';
				}
				routeNums = routeNums +1;
				infowindow.setContent(markerContent);
				infowindow.open(map, this);
...
function fullLoad(){
	...
	promise.then(function(result) {
		...
		/*document.getElementById("addWarningSaved").innerHTML = `<div class="alert alert-success alert-dismissible fade show" role="alert">
																<strong>Paikkojen lataaminen kestää hieman. Kokeile hetken kuluttua uudelleen!<strong>
...
function deleteFromAddress(seqID){
	...
	document.getElementById("addWarningSaved").innerHTML = "";
	document.getElementById("addWarningSaved").innerHTML = `<div class="alert alert-success alert-dismissible fade show" role="alert">
																<strong>Osoite poistettu listalta<strong>
															</div>`
	var card = "add-"+seqID;
